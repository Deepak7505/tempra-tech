import { faBeerMugEmpty, faBottleWater, faBox, faShirt, faMouse, faPaperclip, faWeightHanging, faPen } from "@fortawesome/free-solid-svg-icons";

export const categories = [ 
    {
        categoryName: "Bottles",
        id:"bottles",
        icon:faBottleWater,
        prodcutList : [
            {
                prodId: '00101',
                prodName: "Temperature Bottle",
                prodDetails: "Temperature Bottle – Smart & Stylish Hydration...",
                cardImage:"https://github.com/Deepak7505/myimages/blob/main/bottles-done/bottles-done/tempreture%20bottle%20image/OTHER/5.JPG?raw=true",
                allProductImages: [
                    {img:'https://github.com/Deepak7505/myimages/blob/main/bottles-done/bottles-done/tempreture%20bottle%20image/OTHER/5.JPG?raw=true'},
                    {img:'https://github.com/Deepak7505/myimages/blob/main/bottles-done/bottles-done/tempreture%20bottle%20image/OTHER/6.JPG?raw=true'}, {img:'https://github.com/Deepak7505/myimages/blob/main/bottles-done/bottles-done/tempreture%20bottle%20image/OTHER/7.JPG?raw=true'},
                    {img:'https://github.com/Deepak7505/myimages/blob/main/bottles-done/bottles-done/tempreture%20bottle%20image/OTHER/8.JPG?raw=true'},
                    {img:'https://github.com/Deepak7505/myimages/blob/main/bottles-done/bottles-done/tempreture%20bottle%20image/OTHER/1.jpg?raw=true'}
                    
                ],
                totalDiscription:"The Temperature Bottle is a sleek, insulated smart bottle designed to keep your beverages at the perfect temperature for hours. Featuring a built-in LED temperature display, this bottle allows you to check the temperature of your drink with a simple touch—no more unexpected hot or cold surprises!",
                arrayOfHeighlight:["LED Temperature Display – Smart touch-sensitive screen shows real-time liquid temperature.", "Double-Wall Insulation – Keeps drinks hot for up to 6 hours and cold for up to 12 hours.", "Premium Stainless Steel – Made from 304 food-grade stainless steel, ensuring durability and safety.", "Leak-Proof & BPA-Free – Secure, spill-resistant lid with eco-friendly, non-toxic materials.", "Sleek & Lightweight – Modern design, easy to carry for work, travel, gym, or office use.", "Custom Brandin"]
                
            },
        ],
        productList2:[
            {
                prodId: '00102',
                prodName: "Dori Steel Bottle, 750 mL",
                prodDetails: "Single Wall Dori Water Bottle – Premium Matte Black Finish...",
                cardImage:"https://nestasia.in/cdn/shop/files/waterbottles_3.webp?v=1697635462",
                allProductImages: [{img:'https://nestasia.in/cdn/shop/files/waterbottles_3.webp?v=1697635462'},
                    {img:'https://www.pngmart.com/files/16/Water-Bottle-Flask-Transparent-PNG.png'},
                    {img:'https://www.topvaluereviews.net/wp-content/uploads/2016/05/61lOjVxejWL._SL1200_.jpg'}
                ],
                totalDiscription:"Single Wall Dori Water Bottle – Premium Matte Black Finish Stay hydrated in style with the Single Wall Dori Water Bottle, designed for those who appreciate elegance and functionality. Made from high-quality stainless steel, this bottle features a sleek matte black finish that adds a touch of sophistication. Its leak-proof lid ensures no spills, while the BPA-free material guarantees safe and eco-friendly use.",
                arrayOfHeighlight:["Single Wall  Dori Water Bottle With Premium Quality", "Matt Finish With Black Color", "Leak Proof Lid & BPA Free", "Stainless Steel With Elegant Design","Round Shape & Widen Mouth", "Capacity : 750ml" ],
                subcat:"ss-bottle"
            },
            {
                prodId: '00103',
                prodName: "Matte Black Metal Water Bottle – Stylish & Functional",
                prodDetails: "Stay Hydrated in Style – Sleek. Durable. Essential...",
                cardImage:"https://nestasia.in/cdn/shop/files/waterbottles_3.webp?v=1697635462",
                allProductImages: [{img:'https://nestasia.in/cdn/shop/files/waterbottles_3.webp?v=1697635462'},
                    {img:'https://www.pngmart.com/files/16/Water-Bottle-Flask-Transparent-PNG.png'},
                    {img:'https://www.topvaluereviews.net/wp-content/uploads/2016/05/61lOjVxejWL._SL1200_.jpg'}
                ],
                totalDiscription:"A perfect blend of durability and elegance, this Metal Water Bottle features a matte black finish for a sleek and modern look. Designed with a colored plastic cap and a convenient silicone strap, it’s easy to carry wherever you go.With a 750ml capacity, this bottle is ideal for staying hydrated throughout the day. The engraved branding appears in a striking silver color, adding a premium touch—making it an excellent choice for corporate gifting and promotional giveaways.",
                arrayOfHeighlight:[ "Premium Metal Construction – Durable and long-lasting.", "Matte Black Finish – Sleek and stylish design. Colored Plastic Cap with Silicone Strap – Easy to carry.", "750ml Capacity – Perfect for daily hydration.", "Engraving in Silver – Gives a high-end, professional look.", "Great for Promotions – Ideal for corporate branding and giveaways." ],
                subcat:"ss-bottle"
            },
            {
                prodId: '00104',
                prodName: "500ml Aluminium Bottle",
                prodDetails: "Lightweight Sport Bottle – 500ml White Aluminum...",
                cardImage:"https://nestasia.in/cdn/shop/files/waterbottles_3.webp?v=1697635462",
                allProductImages: [{img:'https://nestasia.in/cdn/shop/files/waterbottles_3.webp?v=1697635462'},
                    {img:'https://www.pngmart.com/files/16/Water-Bottle-Flask-Transparent-PNG.png'},
                    {img:'https://www.topvaluereviews.net/wp-content/uploads/2016/05/61lOjVxejWL._SL1200_.jpg'}
                ],
                totalDiscription:"Stay hydrated on the go with this Lightweight Sport Bottle, crafted from high-quality multicolour aluminum for a sleek and modern look. Weighing just 65 grams and standing at 21 cm in height, this bottle is designed for ultimate portability without compromising on durability.Equipped with a sturdy carabiner, it easily attaches to bags or belts, making it perfect for sports, outdoor adventures, gym sessions, or daily use.",

                arrayOfHeighlight:[ "Ultra-Lightweight (65g) – Easy to carry without extra bulk.", "500ml Capacity – Ideal for staying hydrated throughout the day.", "Premium White Aluminum – Durable and stylish.", "Carabiner Clip – Convenient attachment for backpacks or belts.","Sleek & Compact Design – Perfect for travel, sports, and office use.", "Great for Corporate Branding – Ideal for promotional gifts and giveaways" ],
                subcat:"ss-bottle"
            },
            {
                prodId: '00105',
                prodName: "Vacuum Insulated SS bottle, 550mL",
                prodDetails: "Vacuum Insulated Stainless Steel Slim Fashion Sport Bottle Thermos Tumbler 550ml...",
                cardImage:"https://nestasia.in/cdn/shop/files/waterbottles_3.webp?v=1697635462",
                allProductImages: [{img:'https://nestasia.in/cdn/shop/files/waterbottles_3.webp?v=1697635462'},
                    {img:'https://www.pngmart.com/files/16/Water-Bottle-Flask-Transparent-PNG.png'},
                    {img:'https://www.topvaluereviews.net/wp-content/uploads/2016/05/61lOjVxejWL._SL1200_.jpg'}
                ],
                totalDiscription:"Stay refreshed on the go with this high-quality vacuum-insulated stainless steel sport bottle. Designed to keep liquids hot for up to 10 hours and cold for an impressive 30 hours, this thermos tumbler is perfect for outdoor activities, sports, travel, and daily use. The durable stainless steel interior ensures excellent heat preservation and impact resistance, making it ideal for both beverages and soups. With a strong sealing mechanism, spills and leaks are never a concern.",
                arrayOfHeighlight:["Vacuum Insulation – Maintains temperature for hours (hot: 10h, cold: 30h).", "Premium Stainless Steel – Durable, impact-resistant, and long-lasting.", "550ml Capacity – Perfect size for hydration on the go.", "Multi-Purpose Use – Ideal for beverages and food like soup","Strong Sealing & Leak-Proof – Ensures mess-free usage.", "Sleek & Stylish Design – A modern and practical choice." ],
                subcat:"ss-bottle"
            },
            {
                prodId: '00106',
                prodName: "Duro Steel Stainless Steel Water Bottle – 1000ml",
                prodDetails: "Stainless Steel, Color: Silver and Capacity: 1000 ml....",
                cardImage:"https://nestasia.in/cdn/shop/files/waterbottles_3.webp?v=1697635462",
                allProductImages: [{img:'https://nestasia.in/cdn/shop/files/waterbottles_3.webp?v=1697635462'},
                {img:'https://www.pngmart.com/files/16/Water-Bottle-Flask-Transparent-PNG.png'},
                {img:'https://www.topvaluereviews.net/wp-content/uploads/2016/05/61lOjVxejWL._SL1200_.jpg'}
                ],
                totalDiscription:"The Duro Steel Stainless Steel Water Bottle is a durable and versatile hydration solution designed for everyday use. Crafted from high-quality stainless steel, this 1-liter bottle features a sleek silver finish with a plain yet elegant design. Its single-wall construction makes it lightweight and ideal for storing in the fridge or freezer, ensuring your water stays cool for longer. Whether you're at home, the office, the gym, or traveling, this bottle is a reliable and eco-friendly companion for staying hydrated.",
                arrayOfHeighlight:["Premium Stainless Steel – Durable, rust-resistant, and long-lasting.", "1000ml Capacity – Ample size for all-day hydration.", "Lightweight & Easy to Carry – Perfect for daily use and travel.", "Fridge & Freezer Friendly – Keeps water cool for longer.","Leak-Proof & BPA-Free – Safe, non-toxic, and spill-free design.", "Simple & Elegant Design – Suitable for home, office, and outdoor use." ],
                subcat:"ss-bottle"
            }
        ],
        productList3:[
            
        ],
        img:'https://img.freepik.com/premium-photo/metallic-water-bottle-grey-background_994418-8642.jpg',
    },
    {
        categoryName: "Lunch Box",
        id:"lunchBox",
        icon: faBox,
        prodcutList : [
            {
                prodId: '00201',
                prodName: "Lunch Box 1",
                prodDetails: "description of this product here i will write",
                cardImage:"https://i0.wp.com/bestsellersindia.in/wp-content/uploads/2021/06/Best-Lunch-Box-For-Office-In-India-2.jpg?resize=700%2C700&ssl=1",
                allProductImages: [{img:'https://i0.wp.com/bestsellersindia.in/wp-content/uploads/2021/06/Best-Lunch-Box-For-Office-In-India-2.jpg?resize=700%2C700&ssl=1'}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00202',
                prodName: "Lunch Box 2",
                prodDetails: "description of this product here i will write",
                cardImage:"https://i0.wp.com/bestsellersindia.in/wp-content/uploads/2021/06/Best-Lunch-Box-For-Office-In-India-2.jpg?resize=700%2C700&ssl=1",
                allProductImages: [{img:'https://i0.wp.com/bestsellersindia.in/wp-content/uploads/2021/06/Best-Lunch-Box-For-Office-In-India-2.jpg?resize=700%2C700&ssl=1'}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00203',
                prodName: "Lunch Box 3",
                prodDetails: "description of this product here i will write",
                cardImage:"https://i0.wp.com/bestsellersindia.in/wp-content/uploads/2021/06/Best-Lunch-Box-For-Office-In-India-2.jpg?resize=700%2C700&ssl=1",
                allProductImages: [{img:'https://i0.wp.com/bestsellersindia.in/wp-content/uploads/2021/06/Best-Lunch-Box-For-Office-In-India-2.jpg?resize=700%2C700&ssl=1'}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00204',
                prodName: "Lunch Box 4",
                prodDetails: "description of this product here i will write",
                cardImage:"https://i0.wp.com/bestsellersindia.in/wp-content/uploads/2021/06/Best-Lunch-Box-For-Office-In-India-2.jpg?resize=700%2C700&ssl=1",
                allProductImages: [{img:'https://i0.wp.com/bestsellersindia.in/wp-content/uploads/2021/06/Best-Lunch-Box-For-Office-In-India-2.jpg?resize=700%2C700&ssl=1'}],
                price:234,
                discountedPrize: 190,
                rating:4
            }
        ],
        img:'https://img.freepik.com/premium-photo/workplace-with-lunch-box-gray-textured-table_185193-49182.jpg',
    } ,
    {
        categoryName: "Mugs",
        id:"mugs",
        icon:faBeerMugEmpty,
        prodcutList : [
            {
                prodId: '00301',
                prodName: "Mug 1",
                prodDetails: "description of this product here i will write",
                cardImage:"https://img.freepik.com/premium-photo/mockup-white-mug-nature-background_731930-8628.jpg",
                allProductImages: [{img:'https://img.freepik.com/premium-photo/mockup-white-mug-nature-background_731930-8628.jpg'}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00302',
                prodName: "Mug 2",
                prodDetails: "description of this product here i will write",
                cardImage:"https://img.freepik.com/premium-photo/mockup-white-mug-nature-background_731930-8628.jpg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00303',
                prodName: "Mug 3",
                prodDetails: "description of this product here i will write",
                cardImage:"https://img.freepik.com/premium-photo/mockup-white-mug-nature-background_731930-8628.jpg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00304',
                prodName: "Mug 4",
                prodDetails: "description of this product here i will write",
                cardImage:"https://img.freepik.com/premium-photo/mockup-white-mug-nature-background_731930-8628.jpg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            }
        ],
        img:'https://img.freepik.com/premium-photo/white-mug-mockup-grey-background_921026-10574.jpg',
    },
    {
        categoryName: "Clothes",
        id:"clothes",
        icon:faShirt,
        prodcutList : [
            {
                prodId: '00401',
                prodName: "Clothes 1",
                prodDetails: "description of this product here i will write",
                cardImage:"https://i.etsystatic.com/25501657/r/il/498e7e/3099794773/il_fullxfull.3099794773_422q.jpg",
                allProductImages: [{img:'https://i.etsystatic.com/25501657/r/il/498e7e/3099794773/il_fullxfull.3099794773_422q.jpg'}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00402',
                prodName: "Clothes 2",
                prodDetails: "description of this product here i will write",
                cardImage:"https://i.etsystatic.com/25501657/r/il/498e7e/3099794773/il_fullxfull.3099794773_422q.jpg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00403',
                prodName: "Clothes 3",
                prodDetails: "description of this product here i will write",
                cardImage:"https://i.etsystatic.com/25501657/r/il/498e7e/3099794773/il_fullxfull.3099794773_422q.jpg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00404',
                prodName: "Clothes 4",
                prodDetails: "description of this product here i will write",
                cardImage:"https://i.etsystatic.com/25501657/r/il/498e7e/3099794773/il_fullxfull.3099794773_422q.jpg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            }
        ],
        img:'https://i.etsystatic.com/25501657/r/il/498e7e/3099794773/il_fullxfull.3099794773_422q.jpg',
    },
    {
        categoryName: "Mouse Pad",
        id:"mousePad",
        icon:faMouse,
        prodcutList : [
            {
                prodId: '00501',
                prodName: "Customized Premium Rubber Mousepad –(7.5x9 inches)",
                prodDetails: "Stainless Steel, Color: Silver and Capacity: 1000 ml....",
                cardImage:"https://http2.mlstatic.com/mouse-pad-mouse-D_NQ_NP_764572-MLM31215392938_062019-F.jpg",
                allProductImages: [{img:'https://http2.mlstatic.com/mouse-pad-mouse-D_NQ_NP_764572-MLM31215392938_062019-F.jpg'}],
                totalDiscription:"Upgrade your workspace with this high-quality imported rubber mousepad, designed for smooth and precise mouse movement. With a customizable design, it’s perfect for corporate branding, promotional gifts, or personal use. Crafted from durable rubber, this mousepad ensures a non-slip base for stability, while the smooth surface enhances tracking accuracy for both optical and laser mice.",
                arrayOfHeighlight:["Premium Rubber Material – Ensures durability and long-lasting use.", "Customized Size (7.5x9 inches) – Perfect for work, gaming, or everyday use.", "Non-Slip Base – Provides stability and prevents slipping during use.", "Smooth Surface – Enhances precision for both optical and laser mice.","Imported Quality – Designed for high performance and comfort.", "Great for Custom Branding – Ideal for corporate promotions and gifts." ],
                subcat:"mouse-pad"
            },
            {
                prodId: '00502',
                prodName: "Mouse Pad 2",
                prodDetails: "description of this product here i will write",
                cardImage:"https://http2.mlstatic.com/mouse-pad-mouse-D_NQ_NP_764572-MLM31215392938_062019-F.jpg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00503',
                prodName: "Mouse Pad 3",
                prodDetails: "description of this product here i will write",
                cardImage:"https://http2.mlstatic.com/mouse-pad-mouse-D_NQ_NP_764572-MLM31215392938_062019-F.jpg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00504',
                prodName: "Mouse Pad 4",
                prodDetails: "description of this product here i will write",
                cardImage:"https://http2.mlstatic.com/mouse-pad-mouse-D_NQ_NP_764572-MLM31215392938_062019-F.jpg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            }
        ],
        img:'https://img.freepik.com/premium-photo/computer-mouse-gray-background-3d-rendering_921026-11378.jpg',
    },
    {
        categoryName: "Paper clip board",
        id:"paperClipBoard",
        icon:faPaperclip,
        prodcutList : [
            {
                prodId: '00601',
                prodName: "Paper clip board 1",
                prodDetails: "description of this product here i will write",
                cardImage:"https://5.imimg.com/data5/SELLER/Default/2022/4/XW/GL/UG/46441495/promotional-mdf-clip-board-1000x1000.jpeg",
                allProductImages: [{img:'https://5.imimg.com/data5/SELLER/Default/2022/4/XW/GL/UG/46441495/promotional-mdf-clip-board-1000x1000.jpeg'}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00602',
                prodName: "Paper clip board 2",
                prodDetails: "description of this product here i will write",
                cardImage:"https://5.imimg.com/data5/SELLER/Default/2022/4/XW/GL/UG/46441495/promotional-mdf-clip-board-1000x1000.jpeg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00603',
                prodName: "Paper clip board 3",
                prodDetails: "description of this product here i will write",
                cardImage:"https://5.imimg.com/data5/SELLER/Default/2022/4/XW/GL/UG/46441495/promotional-mdf-clip-board-1000x1000.jpeg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00604',
                prodName: "Paper clip board 4",
                prodDetails: "description of this product here i will write",
                cardImage:"https://5.imimg.com/data5/SELLER/Default/2022/4/XW/GL/UG/46441495/promotional-mdf-clip-board-1000x1000.jpeg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            }
        ],
        img:'https://img.freepik.com/premium-photo/wooden-clipboard-with-blank-a4-paper-mockup-template-isolated-light-grey-background_942243-3386.jpg',
    },
    {
        categoryName: "Paper weight",
        id:"paperWeight",
        icon:faWeightHanging,
        prodcutList : [
            {
                prodId: '00701',
                prodName: "Paper weight 1",
                prodDetails: "description of this product here i will write",
                cardImage:"https://media.rainpos.com/10238/NN_GLASSLRND.jpg",
                allProductImages: [{img:'https://media.rainpos.com/10238/NN_GLASSLRND.jpg'}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00702',
                prodName: "Paper weight 2",
                prodDetails: "description of this product here i will write",
                cardImage:"https://media.rainpos.com/10238/NN_GLASSLRND.jpg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00703',
                prodName: "Paper weight 3",
                prodDetails: "description of this product here i will write",
                cardImage:"https://media.rainpos.com/10238/NN_GLASSLRND.jpg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00704',
                prodName: "Paper weight 4",
                prodDetails: "description of this product here i will write",
                cardImage:"https://media.rainpos.com/10238/NN_GLASSLRND.jpg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            }
        ],
        img:'https://img.freepik.com/premium-photo/paper-weight-white-background_996135-23710.jpg',
    },
    {
        categoryName: "Pen",
        id:"pen",
        icon:faPen,
        prodcutList : [
            {
                prodId: '00801',
                prodName: "Pen 1",
                prodDetails: "description of this product here i will write",
                cardImage:"https://www.qualitylogoproducts.com/custom-pens/satin-pen-hq.jpg",
                allProductImages: [{img:'https://www.qualitylogoproducts.com/custom-pens/satin-pen-hq.jpg'}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00802',
                prodName: "Pen 2",
                prodDetails: "description of this product here i will write",
                cardImage:"https://www.qualitylogoproducts.com/custom-pens/satin-pen-hq.jpg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00803',
                prodName: "Pen 3",
                prodDetails: "description of this product here i will write",
                cardImage:"https://www.qualitylogoproducts.com/custom-pens/satin-pen-hq.jpg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00804',
                prodName: "Pen 4",
                prodDetails: "description of this product here i will write",
                cardImage:"https://www.qualitylogoproducts.com/custom-pens/satin-pen-hq.jpg",
                allProductImages: [{img:''}],
                price:234,
                discountedPrize: 190,
                rating:4
            }
        ],
        img:'https://image.freepik.com/free-psd/black-metal-pen-grey-mockup-template-your-design_34168-1302.jpg',
    }
];


export const homePageTrendConstant = [
    {
        categoryName: "Our Best Selling Products",
        icon:faBottleWater,
        prodcutList : [
            {
                prodId: '00101',
                prodName: "Bottel 1",
                prodDetails: "description of this product here i will write",
                cardImage:"https://nestasia.in/cdn/shop/files/waterbottles_3.webp?v=1697635462",
                allProductImages: [{img:'https://nestasia.in/cdn/shop/files/waterbottles_3.webp?v=1697635462'}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00102',
                prodName: "Mug 1",
                prodDetails: "description of this product here i will write",
                cardImage:"https://img.freepik.com/premium-photo/mockup-white-mug-nature-background_731930-8628.jpg",
                allProductImages: [{img:'https://img.freepik.com/premium-photo/mockup-white-mug-nature-background_731930-8628.jpg'}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00103',
                prodName: "Clothes 1",
                prodDetails: "description of this product here i will write",
                cardImage:"https://i.etsystatic.com/25501657/r/il/498e7e/3099794773/il_fullxfull.3099794773_422q.jpg",
                allProductImages: [{img:'https://i.etsystatic.com/25501657/r/il/498e7e/3099794773/il_fullxfull.3099794773_422q.jpg'}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00104',
                prodName: "Mouse Pad 1",
                prodDetails: "description of this product here i will write",
                cardImage:"https://http2.mlstatic.com/mouse-pad-mouse-D_NQ_NP_764572-MLM31215392938_062019-F.jpg",
                allProductImages: [{img:'https://http2.mlstatic.com/mouse-pad-mouse-D_NQ_NP_764572-MLM31215392938_062019-F.jpg'}],
                price:234,
                discountedPrize: 190,
                rating:4
            }
            
        ],
        img:'https://img.freepik.com/premium-photo/metallic-water-bottle-grey-background_994418-8642.jpg',
    },
    {
        categoryName: "Customizable Products",
        icon:faMouse,
        prodcutList : [
            {
                prodId: '00201',
                prodName: "Bottel 1",
                prodDetails: "description of this product here i will write",
                cardImage:"https://nestasia.in/cdn/shop/files/waterbottles_3.webp?v=1697635462",
                allProductImages: [{img:'https://nestasia.in/cdn/shop/files/waterbottles_3.webp?v=1697635462'}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00202',
                prodName: "Clothes 1",
                prodDetails: "description of this product here i will write",
                cardImage:"https://i.etsystatic.com/25501657/r/il/498e7e/3099794773/il_fullxfull.3099794773_422q.jpg",
                allProductImages: [{img:'https://i.etsystatic.com/25501657/r/il/498e7e/3099794773/il_fullxfull.3099794773_422q.jpg'}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00203',
                prodName: "Mug 1",
                prodDetails: "description of this product here i will write",
                cardImage:"https://img.freepik.com/premium-photo/mockup-white-mug-nature-background_731930-8628.jpg",
                allProductImages: [{img:'https://img.freepik.com/premium-photo/mockup-white-mug-nature-background_731930-8628.jpg'}],
                price:234,
                discountedPrize: 190,
                rating:4
            },
            {
                prodId: '00204',
                prodName: "Lunch Box 4",
                prodDetails: "description of this product here i will write",
                cardImage:"https://i0.wp.com/bestsellersindia.in/wp-content/uploads/2021/06/Best-Lunch-Box-For-Office-In-India-2.jpg?resize=700%2C700&ssl=1",
                allProductImages: [{img:'https://i0.wp.com/bestsellersindia.in/wp-content/uploads/2021/06/Best-Lunch-Box-For-Office-In-India-2.jpg?resize=700%2C700&ssl=1'}],
                price:234,
                discountedPrize: 190,
                rating:4
            }
            
        ],
        img:'https://img.freepik.com/premium-photo/computer-mouse-gray-background-3d-rendering_921026-11378.jpg',
    },
];